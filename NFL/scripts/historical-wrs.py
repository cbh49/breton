...
OUTPUT_JSON = SCRIPT_DIR.parent / "json-data" / "wr_vs_opponent_historical.json"
...
# Respectful scraping
REQUEST_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; wr-scraper/1.0)"
}
REQUEST_TIMEOUT = 15
...
def parse_float(cell_text: str) -> float:
    ...
    if cell_text == "" or cell_text == "-" or cell_text.upper() == "DNP":
        return 0.0
    # strip non-digits, minus, and decimal points
...
def analyze_trend(historical_values: List[float], betting_line: float) -> Optional[str]:
    """
    Analyze trend based on historical values against betting line.
    Returns "OVER" or "UNDER" if trend exists, None otherwise.
    
    Trend requirements:
    - 2 games: both games > line (OVER) or both games < line (UNDER)
    - 3 games: 2 games > line (OVER) or 2 games < line (UNDER)
    - 4 games: 3 games > line (OVER) or 3 games < line (UNDER)
    - 5 games: 3 games > line (OVER) or 3 games < line (UNDER)
    - 6 games: 4 games > line (OVER) or 4 games < line (UNDER)
    - 7+ games: 4 games > line (OVER) or 4 games < line (UNDER)
    """
    ...
    for season in SEASONS:
        ...
        for h in section_headers:
            ...
            for row in rows:
                ...
                # Compute indices based on table type for receiving stats
                i_opp = 1
                # Use different columns for RB vs WR/TE
                if position == "RB":
                ...
                if position == "TE":
                    ...
                    i_rec = 4 + offset  # Receptions for WR/TE
                    i_rec_yds = 6 + offset  # Receiving yards for WR/TE
                ...
                # Parse receiving stats (shifted appropriately for postseason)
...
def main():
    ...
    for player_name, data in all_players.items():
        ...
        if stats and len(stats["opponent"]) >= 2:  # Must have at least 2 matchups to qualify
            # Check if any game has all-zero stats (likely DNP) - skip if so
            for i in range(len(stats["receptions"])):
                if stats["receptions"][i] == 0 and stats["rec_yards"][i] == 0.0:
                    print(f"Skipping {player_name} - has game with all-zero stats (likely DNP)")
                    continue
            
            # Check if all stats are zero - skip if so
            all_receptions_zero = all(rec == 0 for rec in stats["receptions"])
            all_rec_yards_zero = all(yards == 0.0 for yards in stats["rec_yards"])
            
            if all_receptions_zero and all_rec_yards_zero:
                print(f"Skipping {player_name} - all stats are zero")
                continue
            # Get betting lines for this player
            betting_lines = get_player_betting_lines(player_name, receiver_lines)
            if betting_lines:
                # Analyze trends for receptions and receiving yards
                receptions_trend = None
                rec_yards_trend = None
                receptions_rate = None
                rec_yards_rate = None
                
                if betting_lines.get("receptions_line") is not None:
                    receptions_trend = analyze_trend(stats["receptions"], betting_lines["receptions_line"])
                    receptions_rate = calculate_rate(stats["receptions"], betting_lines["receptions_line"])
                
                if betting_lines.get("rec_yards_line") is not None:
                    rec_yards_trend = analyze_trend(stats["rec_yards"], betting_lines["rec_yards_line"])
                    rec_yards_rate = calculate_rate(stats["rec_yards"], betting_lines["rec_yards_line"])
                
                # Only include players who have at least one trend
                if receptions_trend or rec_yards_trend:
                    filtered_stats = {
                        "name": stats["name"],
                        "team": stats["team"],
                    ...
                    # Add relevant stats based on trends
                    if receptions_trend:
                        filtered_stats["receptions"] = stats["receptions"]
                        filtered_stats["receptions_line"] = betting_lines["receptions_line"]
                        filtered_stats["receptions_over"] = betting_lines["receptions_over"]
                        filtered_stats["receptions_under"] = betting_lines["receptions_under"]
                        filtered_stats["receptions_trend"] = receptions_trend
                        filtered_stats["receptions_rate"] = receptions_rate
                    
                    if rec_yards_trend:
                        filtered_stats["rec_yards"] = stats["rec_yards"]
                        filtered_stats["rec_yards_line"] = betting_lines["rec_yards_line"]
                        filtered_stats["rec_yards_over"] = betting_lines["rec_yards_over"]
                        filtered_stats["rec_yards_under"] = betting_lines["rec_yards_under"]
                        filtered_stats["rec_yards_trend"] = rec_yards_trend
                        filtered_stats["rec_yards_rate"] = rec_yards_rate
