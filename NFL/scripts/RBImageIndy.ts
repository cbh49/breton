...
function filterDataByStatType(data: any[], statType: string): any[] {
  return data.filter(item => {
    ...
    const statTypeMap: { [key: string]: string } = {
      'rushrec': 'Rush+Rec Yards',
      'rush_yards': 'Rush Yards',
      'long_rush': 'Longest Rush'
    };
...
async function generateImageForTrend(trendType: string, title: string, filename: string) {
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="utf-8">
      <title>NFL Running Back Props - ${title}</title>
      <style>
        @font-face {
...
async function generateAllImages() {
  const trendConfigs = [
    {
      trendType: 'rushrec',
      title: 'NFL RB RUSH/REC YARDS PROPS',
      filename: 'runningback-rushrec-props.png'
    },
    {
      trendType: 'rush_yards',
      title: 'NFL RB RUSH YARDS PROPS',
      filename: 'runningback-rush-yards-props.png'
    },
    {
      trendType: 'long_rush',
      title: 'NFL RB LONGEST RUSH PROPS',
      filename: 'runningback-longest-rush-props.png'
    }
  ];

  console.log('Starting generation of 3 running back prop images...');
  ...
  console.log('All running back prop images generated successfully!');
}
