...
OUTPUT_JSON = SCRIPT_DIR.parent / "json-data" / "rb_alt_props.json"

# Respectful scraping
REQUEST_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; rb-alt-props-scraper/1.0)"
}
REQUEST_TIMEOUT = 15
...
def modify_alt_line(original_line: float) -> Optional[float]:
    """
    Modify betting lines according to alt line rules:
...
def analyze_player_alt_props(player_name: str, team: str, position: str, url: str, rb_lines: List[Dict], public_bets: List[Dict], team_abbreviations: Dict[str, str]) -> Optional[Dict]:
    """
    ...
    # Only check rush_yards_line and rushrec_line
    ...
    alt_rush_yards_line = modify_alt_line(rush_yards_line) if rush_yards_line else None
    alt_rushrec_line = modify_alt_line(rushrec_line) if rushrec_line else None
    
    # Check if player qualifies for either stat
    if alt_rush_yards_line and is_over_line_in_all_games(games, "rush_yards", alt_rush_yards_line):
        has_qualifying_stats = True
    elif alt_rushrec_line and is_over_line_in_all_games(games, "rushrec_yards", alt_rushrec_line):
        has_qualifying_stats = True
    ...
    if has_qualifying_stats:
        ...
        if alt_rush_yards_line and is_over_line_in_all_games(games, "rush_yards", alt_rush_yards_line):
            player_data.update({
                "rush_yards_line": rush_yards_line,  # Original line
                "rush_yards_alt_line": alt_rush_yards_line,  # Modified alt line
        ...
        if alt_rushrec_line and is_over_line_in_all_games(games, "rushrec_yards", alt_rushrec_line):
            player_data.update({
                "rushrec_line": rushrec_line,  # Original line
                "rushrec_alt_line": alt_rushrec_line,  # Modified alt line
...
def main():
    ...
    for player_name, data in rb_players.items():
        ...
        player_data = analyze_player_alt_props(player_name, team, position, url, rb_lines, public_bets, team_abbreviations)
        if player_data:
